![header](https://capsule-render.vercel.app/api?type=waving&color=auto&height=250&section=header&text=Lee_KangYub's&fontSize=60&desc=profile%20&descAlign=78)

[🇰🇷 Korean Version](/ko/) | [🇺🇸 English Version](/en/)

## 👨‍💻 소개
- 기구부, 전장부를 비롯한 HW 설계부터 SW 프로그래밍 및 구동까지 모바일 로봇의 End-to-End 개발 경험 보유
- 특수 환경용 로봇 개발과 ROS2 기반 임베디드 시스템 구현에 2년 이상의 경험을 보유 
- 로프 등강 로봇, 모바일 앵커 로봇 등 비정형 환경에서 작동하는 모바일 로봇 개발
- 졸업 프로젝트 : 로프라는 특수한 환경 속 모바일 로봇의 잔류진동 제어 연구로 국제 SCI 논문 게재
- 국내 등록 특허 2건 보유
- 국제 SCI 논문 3건 게재

## 🔑 핵심 역량
- **메카트로닉스 시스템 설계 및 통합**
- **HW - SW 인터페이스 설계**
- **임베디드 시스템 프로그래밍 (C/Python/Linux)**
- **ROS2 기반 로봇 제어 시스템 구현**
- **진동 제어 알고리즘 개발**

## 🛠️ 기술 스택
- **로봇 개발**: ROS2(중급), Linux(중급), Kinematics & Dynamics(고급)
- **프로그래밍**: C(중급), Python(중급), MATLAB(고급)
- **설계 TOOL**: Autodesk Inventor(고급), CATIA(중급), KiCAD(중급)
- **임베디드 시스템**: MCU(ESP32, Arduino), SoC(Raspberry Pi), CAN
- **시뮬레이션**: MATLAB & Simulink(고급)
  
## 🎓 학력
### 가천대학교 | 2017.03 - 2019.02
- **전공**: 바이오나노학과
- **GPA**: **4.16/4.5**
- **비고**: 한양대학교 기계공학부로 편입

### 한양대학교 | 2019.03 - 2023.02
- **학위**: 학사
- **전공**: 기계공학부
- **GPA**: **3.94/4.5**
 
### 한양대학교 공학기술대학원 | 2023.03 - 2025.02
- **학위**: 석사
- **전공**: 융합기계공학과
- **GPA**: **4.38/4.5**
- **지도교수**: 서태원
- **소속연구실**: 로봇설계공학연구실(RODEL)
- **연구 주제**: 로프 기반 다중 로봇 시스템의 잔류 진동 저감을 위한 제어 알고리즘 개발
- **비고**: 국외 SCI논문 3개 게재 및 특허 3개 출원
  
## 🌐 언어 능력
- **영어**: TOEIC Speaking - **Advanced Low** (2024.08)
  

## 📜 특허
### 등록
- **Synchronized wheel leg를 적용한 홈서비스 모바일 로봇 플랫폼 메커니즘** 
  - 등록날짜: 2024.12
  - 등록번호: **10-2742903**
  - 핵심 기술: 5자유도(바퀴 구동 모터 4개, 다리 구동 모터 1개)로 단순화된 실내 서비스 로봇 메커니즘

- **캐터필러 트랙과 4절 링크 구조를 사용한 로프 라이딩 메커니즘**
  - 등록날짜: 2025.09
  - 등록번호: **10-2858452**
  - 핵심 기술: 80kg 하중의 로봇을 지지하면서 로프 위를 주행할 수 있는 혁신적 메커니즘
    
### 출원
- **가변 강성 유연 소재 바퀴** 
  - 출원날짜: 2023.05
  - 출원번호: **10-2023-0057616**
  - 핵심 기술: 환경에 따라 강성을 자동 조절하여 험지와 평지 모두에서 최적 성능 발휘





## 🏆 수상 실적
- **한양대학교 종합 설계 발표회 우수상** | 2022.05
  - '장애물 극복이 가능한 라스트마일 배달 로봇' 개발
  - 2022학년도 1학기  한양대학교 기계공학종합 설계 발표대회 우수상을 수상함
 
## 📝 기술 교육
- **현대엔지비 H-모빌리티 클래스 [자율주행]** | 2025.4 ~ 2025.7
  - 자율주행 Track_인지, 판단, 제어 수료
  - 자율주행 Track_판단 심화 수료 : A* 알고리즘, RRT 알고리즘, SLAM

- **두산 ROKEY - Robotics bootcamp** | 2025. 7 ~ ING...
  - AI - Computer Vision
  - ROS2
  - 협동로봇 및 지능형로봇 프로그래밍 실습


## 📑 연구 논문

### 주저자
- **Rope on Rope: Reducing residual vibrations in rope-based anchoring system and rope-driven facade operation robot**
  - 저널: **Sensors** (SCI)
  - 게재일: 2025.04.14
  - 기여:
    - 로봇 HW 설계 및 구동 SW 프로그래밍
    - 로프기반 모바일 로봇의 잔류진동 제어 알고리즘 개발 및 검증

### 공동저자
- **The stiffness adjustable wheel mechanism based on compliant spoke deformation**
  - 저널: **Scientific Reports** (SCI)
  - 게재일: 2024.01.08
  - 저자 순위: 2위 (총 5명)
  - 기여: 가변 강성 휠 메커니즘의 프로토타입 개발 및 실험적 검증
    
- **Rope-riding mobile anchor for robots operating on convex facade**
  - 저널: **Sensors** (SCI)
  - 게재일 : 2025.07.29
  - 저자 순위: 2위 (총 4명)
  - 기여: 로프 위를 주행하는 모바일 로봇의 HW 설계 및 구동 SW 프로그래밍
 
    
## 📚 참여 연구 과제

### **한국연구재단 과제**
- **다양한 형태의 외벽에서 자유롭게 이동이 가능한 로봇 플랫폼 개발**  
  - 기간 : 2023.03 - 2024.12
  - 역할: 모바일 로봇 HW 설계 및 구동 SW 개발
  - 성과: 비정형 건물 외벽 청소 로봇의 **작업 영역 확장**
  - **전력 소모 최대 58.3% 감소 및 조작도 약 53% 향상**

- **환경 적응형 스마트 휠 기반 로봇 시스템 연구** 
  - 기간 :  2022.03 - 2023.12
  - 역할: 모바일 로봇 성능 평가용 테스트 벤치 설계 및 실험 데이터 분석
  - 성과: 환경에 따라 강성을 조절해 주행하는 가변 강성 휠 개발

- **탄소중립 ESG 미래선도 인재양성을 위한 실전문제연구단** 
  - 기간 : 2022.03 - 2022.12
  - 역할: 무게중심 조절을 이용한 서비스 로봇 플랫폼 HW 설계 및 SW 개발
  - 성과: 안정성 향상된 서비스 로봇 플랫폼 구현
  
## 💼 주요 프로젝트 및 성과

### **Façade cleaning robot project** | 2023.03 - 2024.12

- **성과**:
  - 비정형 형상 건물(*고척스카이돔)에 **적용 가능한 로봇 시스템을 개발**하여 기존 방식 대비 **작업 영역을 40% 확장**
  - 개발한 로봇 플랫폼으로 **고척 스카이돔 현장 테스트에서 95% 성공률** 달성
  - **2023, 2024 로보월드 전시회**에서 기술 시연 및 업계 관심 유치
- **기술적 성과**:
  - 로프와 Wheel-Leg 메커니즘을 융합하여 **고경사 장애물(45° 이상) 극복**
  - GPS 및 센서 기반 위치추정알고리즘으로 **실시간 로봇 위치 추적 및 제어 정확도 90% 이상** 확보
- **개발 시스템**:
  - **DAR (Dual Ascending Robot)**: 곡면 환경에서 로프등강 이동 플랫폼
  - **RDWR (Rope Driven Wheel-Leg Robot)**: 4족 휠레그 구조로 외벽 장애물 극복 능력 향상
  - **RMA (Rope-Riding Mobile Anchor)**: 작업 영역 확장용 이동식 앵커 포인트
    
<div align="center">
<img src="https://github.com/user-attachments/assets/c6cf36b8-74a6-4e57-b0e2-861994493488" width="600" height="300">
</div>
<div align="center">
그림1. 외벽 청소 로봇의 고척스카이돔 현장 테스트 
</div>
<div align="center">
<img src="https://github.com/user-attachments/assets/d9dcf2d0-2baa-4ae6-8e52-de3b6f2d62ea" width="600" height="600">
</div>
<div align="center">
그림2. 경사면 청소 및 장애물 극복
</div>

### **RMA (Rope-Riding Mobile Anchor) 시스템 개발** | 2023.01 - 2023.12
- **성과**:
  - **50.5kg 하중**을 지지하면서도 안정적 로프 주행 구현
  - **49.5kN의 외력**에도 위치 유지 가능한 고성능 앵커 메커니즘 개발
  - 기존 고정식 앵커 대비 **RMA 이동거리에 비례해 DAR 작업 영역 확장**
  - **전력 소모 최대 58.3% 감소 및 조작도 약 53% 향상** 달성
- **기술적 성과**:
  - 캡스턴 방정식 기반 로프 고정 메커니즘 구현으로 **안정성과 이동성 동시 확보**
  - MCU와 CAN 네트워크 기반 센서 통합으로 **정밀한 위치 추정 및 제어 구현**
  - **특허 출원 완료 (출원번호: 10-2023-0126021)**
  - **SCI 저널 논문 투고 (Robotics and Autonomous Systems)**

<div align="center">
<img src="https://github.com/user-attachments/assets/2a5429c2-dda2-4379-9888-99c128ec063b" width="600" height="300">
</div>
<div align="center">
그림1. 로프 위를 주행하는 RMA
</div>

<div align="center">
<img src="https://github.com/user-attachments/assets/eeaf0662-8083-486b-8d63-109fc345da43" width="1000" height="400">
</div>
<div align="center">
그림2. RMA를 활용한 DAR의 작업영역 및 조작도 향상 실험
</div>

<div align="center">
 Youtube_Video : https://www.youtube.com/watch?v=r_9pKClJhF0&t=36s
</div>

### **DAR (Dual Ascending Robot) 잔류 진동 제어** | 2024.01 - 2024.12
- **성과**:
  - 하드웨어의 잔류 진동 제어 목적의 **RVCA(Residual Vibration Control Algorithm)** 개발
  - 알고리즘 적용 결과 잔류진동 **최대진폭 40.29% 감소, 정정시간 54.02% 단축**
- **기술적 성과**:
  - 로프를 Spring-Damper로 모델링한 **시스템Identification 및 동역학 분석** 수행
  - Input Shaping 제어와 Disturbance Observer를 결합한 **RVCA 제어 알고리즘** 개발
  - **ROS2와 RTOS 통합 프로그래밍**으로 실시간 진동 제어 구현
  - **SCI 저널 논문 투고 (Sensors)**
    
<div align="center">
<img src="https://github.com/user-attachments/assets/203aed7e-9425-4914-9423-4491b34f2dad" width="1000" height="350">
</div>
<div align="center">
그림1. 소형 DAR과 진동제어 실험 개요
</div>

<div align="center">
<img src="https://github.com/user-attachments/assets/44152df5-bac9-4162-b81d-ea646a0236db" width="900" height="450">
</div>
<div align="center">
그림2. 진동제어 알고리즘 비교 실험 결과
</div>

<div align="center">
 Youtube_Video : https://www.youtube.com/watch?v=hb9WydOYG6A
</div>


## 📞 연락처
- **이메일**: amogae7@gmail.com 
- **LinkedIn**: [linkedin.com/in/강엽-이-31254836b](https://www.linkedin.com/in/강엽-이-31254836b/)
<!-- - **GitHub**: [github.com/amogae7](https://github.com/amogae7) -->
